...
void VoronoiLayer::onInitialize()
{
  ...
  dynamic_reconfigure::Server<costmap_2d::GenericPluginConfig>::CallbackType cb = boost::bind(
      &VoronoiLayer::reconfigureCB, this, _1, _2);
...
void VoronoiLayer::updateCosts(costmap_2d::Costmap2D& master_grid, int min_i, int min_j, int max_i,
                                          int max_j)
{
  if (!enabled_)
    return;
  ...
  if((max_i-min_i) >= size_x_ && (max_j-min_j) >= size_y_){	  
	  ...
	  //if(obstacles_present){
		  ROS_INFO("UPDATING VORONOI COSTMAP!!  (%i X %i)", size_x_,size_y_);
